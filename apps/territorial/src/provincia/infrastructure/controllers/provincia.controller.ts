...
export class ProvinciaController {
  ...
  create(@Body() data: any, @Req() req: any) {
    return this.createProvinciaUseCase.execute(data, req.user?.id, req.user?.tenantId);
  }

  @Put(':id')
  @CheckPolicies((ability: any) => ability.can('update', 'Provincia'))
  updatePut(@Param('id') id: string, @Body() data: any, @Req() req: any) {
    return this.updateProvinciaUseCase.execute(id, data, req.user?.id, req.ability);
  }
