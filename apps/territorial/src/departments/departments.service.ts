...
export class DepartmentsService {
  ...
  async create(createDepartmentDto: CreateDepartmentDto, user: any) {
    try {
      // Validaciones
      if (!createDepartmentDto.nombre || createDepartmentDto.nombre.trim() === '') {
        throw new BadRequestException('El nombre del departamento es requerido');
      }

      if (!createDepartmentDto.abreviacion || createDepartmentDto.abreviacion.trim() === '') {
        throw new BadRequestException('La abreviación del departamento es requerida');
      }
      ...
      if (existing) {
        throw new BadRequestException(`Ya existe un departamento con el nombre "${createDepartmentDto.nombre}"`);
      }
      ...
      this.logger.log(`Departamento creado: ${dep.nombre} (ID: ${dep.id}) por usuario ${user?.username || 'desconocido'}`);
      ...
      throw new InternalServerErrorException('Error al crear el departamento. Por favor, intente nuevamente');
    }
  }
  ...
  async findAll() {
    try {
      ...
      throw new InternalServerErrorException('Error al obtener la lista de departamentos');
    }
  }
  ...
  async findOne(id: string) {
    try {
      ...
      if (!dep) {
        throw new NotFoundException(`No se encontró el departamento con ID ${id} o fue eliminado`);
      }
      ...
      throw new InternalServerErrorException('Error al obtener el departamento');
    }
  }
   ...
   */
  async update(id: string, updateDepartmentDto: UpdateDepartmentDto, user: any) {
    try {
      if (!id) {
        throw new BadRequestException('ID de departamento requerido');
      }
      ...
      if (updateDepartmentDto.nombre && updateDepartmentDto.nombre !== existing.nombre) {
        ...
        if (duplicate) {
          throw new BadRequestException(`Ya existe otro departamento con el nombre "${updateDepartmentDto.nombre}"`);
        }
      }
      ...
      const dep = await this.prisma.departamento.update({
        where: { id: id },
        data: {
          ...(updateDepartmentDto.nombre && { nombre: updateDepartmentDto.nombre.trim() }),
          ...(updateDepartmentDto.abreviacion && { abreviacion: updateDepartmentDto.abreviacion.trim() }),
          updatedBy: user?.id || null,
        },
      });

      await this.audit('UPDATE', 'departamento', dep.id, user, { old: existing, new: dep });

      this.logger.log(`Departamento actualizado: ${dep.nombre} (ID: ${dep.id}) por usuario ${user?.username || 'desconocido'}`);
  ...
  async remove(id: string, user: any) {
    try {
      ...
      await this.audit('DELETE', 'departamento', dep.id, user, { deleted: existing });

      this.logger.log(`Departamento eliminado: ${dep.nombre} (ID: ${dep.id}) por usuario ${user?.username || 'desconocido'}`);
   ...
   * Registrar auditoría
   */
  private async audit(action: string, resource: string, resourceId: string, user: any, details: any) {
    try {
