...
import { IGaleriaRepository } from '../../domain/repositories/galeria.repository.interface';

@Injectable()
export class PrismaGaleriaRepository implements IGaleriaRepository {
  constructor(
  ...
  async findAll(filter: any = {}, ability?: any): Promise<any[]> {
    ...
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'read', 'Galeria');
      where = { AND: [where, caslWhere] };
    }

    return await (this.prisma as any).galeria.findMany({
      where,
  ...
  async delete(id: string, userId?: string, ability?: any): Promise<void> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'delete', 'Galeria');
      where = { AND: [where, caslWhere] };
    }
    const exists = await (this.prisma as any).galeria.findFirst({ where });
    if (!exists)
    ...
    const hasStatus = true;
    if (hasStatus) {
      await (this.prisma as any).galeria.update({
        where: { id },
        data: { estado: 'eliminado', deletedAt: new Date(), deletedBy: userId },
      });
    } else {
      await (this.prisma as any).galeria.delete({ where: { id } });
    }
  }
}
