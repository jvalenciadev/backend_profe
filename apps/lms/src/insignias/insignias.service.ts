...
export class InsigniasService {
  ...
  async otorgarInsigniaManual(
    ...
    if (!isFac)
      throw new ForbiddenException(
        'Solo un facilitador puede otorgar insignias',
      );
  ...
  async revocarInsignia(
    ...
    if (!isFac)
      throw new ForbiddenException(
        'Solo un facilitador puede revocar insignias',
      );
    ...
    if (!record)
      throw new NotFoundException('El estudiante no tiene esa insignia');
  ...
  async getInsigniasPorModulo(moduloId: string, turnoId?: string) {
    ...
    const modMaestro = !modOper
      ? await this.prisma.programaModulo.findUnique({
          where: { id: moduloId },
        })
      : null;
  ...
  async deleteInsignia(id: string) {
    ...
    await this.prisma.mod_insignia_user.deleteMany({
      where: { insigniaId: id },
    });
