...
export class RecordatoriosService {
    ...
    async handleCronReminders() {
        this.logger.log('LMS: Verificando actividades pendientes para recordatorios...');
        try {
    ...
    private async notifyPendingStudents(act: any, threshold: string, message: string, label: string) {
        ...
        const studentIds = Array.from(new Set(inscripciones.map(i => i.personaId)));
        ...
        try {
            ...
            if (act.tipo === 'CUESTIONARIO') {
                const intento = await this.prisma.mod_intento.findFirst({
                    where: { cuestionario: { actividadId: act.id }, userId, estado: 'finalizado' }
                });
                return !!intento;
            }
            if (act.tipo === 'FORO') {
            ...
            this.logger.error(`Error verificando entrega para ${act.id}/${userId}:`, e);
        }
        return false;
    }
}
