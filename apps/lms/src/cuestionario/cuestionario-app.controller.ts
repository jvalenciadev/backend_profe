...
export class CuestionarioAppController {
  constructor(private readonly cuestionarioAppService: CuestionarioAppService) { }

  @Get('info/:cuestionarioId')
  async getCuestionarioInfo(@Param('cuestionarioId') cuestionarioId: string, @Request() req: any) {
    return this.cuestionarioAppService.getInfo(cuestionarioId, req.user.id);
  }

  @Post('iniciar/:cuestionarioId')
  async iniciarIntentoApp(@Param('cuestionarioId') cuestionarioId: string, @Request() req: any) {
    ...
    const isMobile = req.headers['user-agent']?.match(/Android|iPhone|iPad|iPod/i) !== null;

    // Obtenemos info para verificar soloMobile
    const info = await this.cuestionarioAppService.getInfo(cuestionarioId, req.user.id);

    if (info.cuestionario.soloMobile && !isMobile) {
      if (!req.headers['x-app-platform']?.match(/android|ios|flutter/i)) {
        throw new UnauthorizedException('Este cuestionario solo puede ser realizado desde la App Móvil (Obliga a realizar el cuestionario desde celular o tablet)');
      }
    }

    return this.cuestionarioAppService.iniciarIntento(cuestionarioId, req.user.id);
  }
  ...
  async guardarProgreso(
    ...
    @Request() req: any,
    @Body() body: { respuestas: { preguntaId: string; opcionId?: string; textoLibre?: string }[] }
  ) {
    ...
    return this.cuestionarioAppService.guardarProgreso(intentoId, req.user.id, body.respuestas || []);
  }
  ...
  async finalizarIntento(
    ...
    return this.cuestionarioAppService.finalizarIntento(intentoId, req.user.id, body.motivoBloqueo);
  }
}
