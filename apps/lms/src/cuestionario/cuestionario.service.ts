...
export class CuestionarioService {
    ...
    async getIntentosPorCuestionario(cuestionarioId: string) {
        return this.prisma.mod_intento.findMany({
            ...
            include: {
                user: { select: { id: true, nombre: true, apellidos: true, correo: true } },
                respuestas: true
            },
                ...
                for (const opt of p.opciones) {
                    if (opt.id && !opt.isNew) {
                        await this.prisma.mod_opcion.update({
                            where: { id: opt.id },
                            data: { texto: opt.texto, esCorrecta: opt.esCorrecta, orden: opt.orden }
                        });
                    } else {
    ...
    async getLobbyData(userId: string, cuestionarioId: string) {
        ...
        return {
            ...
            intentosRestantes: Math.max(0, cue.maxIntentos - (enProgreso ? intentos.length - 1 : intentos.length)),
    ...
    async iniciarIntento(userId: string, cuestionarioId: string) {
        ...
        let intentoEnProgreso = await this.prisma.mod_intento.findFirst({
    ...
    async guardarRespuesta(intentoId: string, data: { preguntaId: string, opcionId?: string, textoLibre?: string }) {
    ...
    async finalizarIntento(intentoId: string) {
        ...
        for (const res of intento.respuestas) {
            const pregunta = intento.cuestionario.preguntas.find(p => p.id === res.preguntaId);
        ...
        const maxPosibleIntento = intento.respuestas.reduce((sum, res) => {
            const p = intento.cuestionario.preguntas.find(pre => pre.id === res.preguntaId);
            return sum + (p?.puntaje || 0);
        }, 0);

        let notaMapeada = notaObtenida;
        if (actividad && maxPosibleIntento > 0 && maxPosibleIntento !== actividad.puntajeMax) {
