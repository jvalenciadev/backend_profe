...
// ──────────────────────────────────────────────────────────────────────────────
const QR_SECRET =
  process.env.QR_ASISTENCIA_SECRET || 'qr_asistencia_secret_2024_profe';
...
export class AsistenciaService {
  ...
  async marcarAsistenciaQR(userId: string, token: string) {
    ...
    if (parts.length !== 5)
      throw new BadRequestException('Token QR mal formado');
    ...
    if (Date.now() > expiry) {
      throw new BadRequestException(
        'El código QR ha expirado. Solicite uno nuevo al facilitador.',
      );
    }
  ...
  async crearSesion(
    userId: string,
    targetId: string,
    data: { fecha: string; turnoId?: string; esPresencial?: boolean },
  ) {
    // Identificar si es módulo operativo o maestro
    const modOper = await this.prisma.programaModuloDos.findUnique({
      where: { id: targetId },
    });
    const modMaestro = !modOper
      ? await this.prisma.programaModulo.findUnique({ where: { id: targetId } })
      : null;

    if (!modOper && !modMaestro)
    ...
    if (!isFacilitador)
      throw new ForbiddenException(
        'No tienes permisos de facilitador para este módulo',
      );
    ...
    if (
      !finalTurnoId &&
      isFacilitador &&
      (isFacilitador as any).turnoId &&
      !modMaestro
    ) {
      finalTurnoId = (isFacilitador as any).turnoId;
    }

    // Evitar duplicados para la misma fecha y turno (Normalizar fecha a medianoche UTC)
    const normalizedDate = new Date(`${data.fecha}T00:00:00.000Z`);
    ...
    return this.prisma.mod_asistencia.create({
      data: {
        ...
        fecha: normalizedDate,
        esPresencial:
          data.esPresencial !== undefined ? data.esPresencial : true,
      },
    });
  }
  ...
  async getRegistrosSesion(userId: string, asistenciaId: string) {
    ...
    // Get students
    const whereInscripcion: any = { estado: 'activo' };
    ...
    const turnos = await this.prisma.programaDosTurno.findMany({
      where: {
        id: {
          in: inscripciones.map((i) => i.turnoId).filter(Boolean) as string[],
        },
      },
      include: { turnoConfig: true },
    });
