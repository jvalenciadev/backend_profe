...
function buildValidToken(sesionId = 'sesion-1', turnoId = 'turno-1', sedeId = 'sede-1', offsetMs = 60000): string {
...
describe('AsistenciaService (Blindaje Completo)', () => {
  ...
  const mockPrisma = {
    mod_asistencia: { findUnique: jest.fn(), findMany: jest.fn(), findFirst: jest.fn(), create: jest.fn() },
    mod_asistencia_reg: { findFirst: jest.fn(), findMany: jest.fn(), create: jest.fn(), upsert: jest.fn() },
  ...
  describe('generateQrToken', () => {
    it('debería lanzar NotFoundException si la sesión no existe', async () => {
      ...
      await expect(service.generateQrToken('user-1', 'sesion-999'))
        .rejects.toThrow(NotFoundException);
    });

    it('debería lanzar ForbiddenException si el usuario no es facilitador', async () => {
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue({ id: 'ses-1', moduloId: 'mod-1', moduloMaestroId: null, turnoId: null, modulo: null, moduloMaestro: null });
      mockPrisma.programaDosFacilitador.findFirst.mockResolvedValue(null);
      await expect(service.generateQrToken('user-sin-permisos', 'ses-1'))
    ...
    it('debería generar token QR válido si el usuario es facilitador', async () => {
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue({
        id: 'ses-1', moduloId: 'mod-1', moduloMaestroId: null, turnoId: 'turno-1',
        modulo: { programaDos: { sedeId: 'sede-1' } }, moduloMaestro: null,
      });
      mockPrisma.programaDosFacilitador.findFirst.mockResolvedValue({ id: 'fac-1' });
  ...
  describe('marcarAsistenciaQR', () => {
    it('debería lanzar BadRequestException si el token QR es inválido', async () => {
      await expect(service.marcarAsistenciaQR('user-1', 'token_invalido_base64!!'))
        .rejects.toThrow(BadRequestException);
    });

    it('debería lanzar BadRequestException si el QR ha expirado', async () => {
      // Token con expiry en el pasado
      const expiredToken = buildValidToken('ses-1', 'turno-1', 'sede-1', -10000);
      await expect(service.marcarAsistenciaQR('user-1', expiredToken))
        .rejects.toThrow(BadRequestException);
    });
    ...
    it('debería lanzar NotFoundException si la sesión del QR no existe en BD', async () => {
      ...
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue(null);
      await expect(service.marcarAsistenciaQR('user-1', validToken))
        .rejects.toThrow(NotFoundException);
    });
    ...
    it('debería lanzar ForbiddenException si el estudiante no está inscrito en el turno', async () => {
      ...
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue({ id: 'ses-1', moduloId: 'mod-1', modulo: { programaDos: { id: 'prog-1' } } });
    ...
    it('debería retornar alreadyRegistered=true si la asistencia ya existe', async () => {
      const validToken = buildValidToken('ses-1', 'turno-1', 'sede-1');
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue({ id: 'ses-1', moduloId: 'mod-1', actividadId: null, modulo: { programaDos: { id: 'prog-1' } } });
      mockPrisma.programaInscripcion.findFirst.mockResolvedValue({ id: 'ins-1' });
      mockPrisma.mod_asistencia_reg.findFirst.mockResolvedValue({ id: 'reg-1' });
    ...
    it('debería registrar asistencia exitosamente (nuevo registro)', async () => {
      ...
      expect(mockPrisma.mod_asistencia_reg.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ estado: 'P', userId: 'user-1' }) })
      );
    });
  });
  ...
  describe('getSesionesModulo', () => {
    it('debería retornar sesiones con turnoNombre hidratado', async () => {
      mockPrisma.mod_asistencia.findMany.mockResolvedValue([
        { id: 'ses-1', turnoId: 'turno-1', _count: { registros: 5 }, modulo: null, moduloMaestro: null, actividad: null },
      ]);
      mockPrisma.programaDosTurno.findUnique.mockResolvedValue({ turnoConfig: { nombre: 'Turno Mañana' } });
    ...
    it('debería retornar turnoNombre=Global si no hay turnoId', async () => {
      mockPrisma.mod_asistencia.findMany.mockResolvedValue([
        { id: 'ses-2', turnoId: null, _count: { registros: 0 }, modulo: null, moduloMaestro: null, actividad: null },
      ]);
  ...
  describe('crearSesion', () => {
    it('debería lanzar NotFoundException si el módulo no existe', async () => {
      ...
      await expect(service.crearSesion('user-1', 'mod-999', { fecha: '2024-01-01' }))
        .rejects.toThrow(NotFoundException);
    });
    ...
    it('debería lanzar ForbiddenException si no es facilitador', async () => {
      ...
      mockPrisma.programaDosFacilitador.findFirst.mockResolvedValue(null);
      await expect(service.crearSesion('user-sin-permisos', 'mod-1', { fecha: '2024-01-01' }))
        .rejects.toThrow(ForbiddenException);
    });

    it('debería retornar sesión existente si ya hay una para esa fecha y turno', async () => {
      mockPrisma.programaModuloDos.findUnique.mockResolvedValue({ id: 'mod-1' });
      mockPrisma.programaDosFacilitador.findFirst.mockResolvedValue({ id: 'fac-1', turnoId: 'turno-1' });
      mockPrisma.mod_asistencia.findFirst.mockResolvedValue({ id: 'existing-ses' });

      const result = await service.crearSesion('facilitador-1', 'mod-1', { fecha: '2024-01-15', turnoId: 'turno-1' });
    ...
    it('debería crear nueva sesión si no existe una para esa fecha', async () => {
      ...
      const result = await service.crearSesion('facilitador-1', 'mod-1', { fecha: '2024-02-20', turnoId: 'turno-1' });
  ...
  describe('registrarAsistencia', () => {
    it('debería lanzar NotFoundException si la sesión no existe', async () => {
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue(null);
      await expect(service.registrarAsistencia('user-1', 'ses-999', { registros: [] }))
        .rejects.toThrow(NotFoundException);
    });

    it('debería lanzar ForbiddenException si no tiene permisos de facilitador', async () => {
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue({ id: 'ses-1', moduloId: 'mod-1', moduloMaestroId: null });
      mockPrisma.programaDosFacilitador.findFirst.mockResolvedValue(null);
      await expect(service.registrarAsistencia('user-1', 'ses-1', { registros: [] }))
        .rejects.toThrow(ForbiddenException);
    });

    it('debería registrar asistencia y retornar success=true', async () => {
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue({ id: 'ses-1', moduloId: 'mod-1', moduloMaestroId: null, actividadId: null });
      mockPrisma.programaDosFacilitador.findFirst.mockResolvedValue({ id: 'fac-1' });
      mockPrisma.mod_asistencia_reg.upsert.mockResolvedValue({});

      const registros = [{ registroId: null, userId: 'student-1', estado: 'P', observacion: '' }];
      const result = await service.registrarAsistencia('fac-1', 'ses-1', { registros });
    ...
    it('debería sincronizar notas con las asistencias (P=100%, T=50%, F=0%)', async () => {
      mockPrisma.mod_asistencia.findUnique.mockResolvedValue({ id: 'ses-1', moduloId: 'mod-1', moduloMaestroId: null, actividadId: 'act-1' });
      mockPrisma.programaDosFacilitador.findFirst.mockResolvedValue({ id: 'fac-1' });
      mockPrisma.mod_asistencia_reg.upsert.mockResolvedValue({});
      mockPrisma.mod_actividad.findUnique.mockResolvedValue({ puntajeMax: 100 });
      ...
      const calls = (mockPrisma.mod_nota_actividad.upsert as jest.Mock).mock.calls;
  ...
  describe('getAsistenciaEstudiante', () => {
    it('debería retornar sesiones con estado F si el estudiante no tiene registro', async () => {
      mockPrisma.programaInscripcion.findFirst.mockResolvedValue({ turnoId: 'turno-1' });
      mockPrisma.mod_asistencia.findMany.mockResolvedValue([
        { id: 'ses-1', fecha: new Date(), esPresencial: true, registros: [] },
      ]);

      const result = await service.getAsistenciaEstudiante('student-1', 'mod-1');
    ...
    it('debería retornar el estado real del registro si el estudiante asistió', async () => {
      mockPrisma.programaInscripcion.findFirst.mockResolvedValue({ turnoId: null });
      mockPrisma.mod_asistencia.findMany.mockResolvedValue([
        { id: 'ses-1', fecha: new Date(), esPresencial: true, registros: [{ estado: 'P', userId: 'student-1' }] },
      ]);
