...
export class GradingService {
    ...
    private async verifyFacilitador(userId: string, moduloId: string) {
        ...
        const asignacionMaestro = await this.prisma.programaDosFacilitador.findFirst({
            where: { facilitadorId: userId, moduloMaestroId: moduloId, estado: 'activo' },
            include: {
                programaDos: {
                    include: { tipo: true },
                },
        ...
        if (masterMod) {
            ...
            return {
                programaDos: { tipoId: masterMod.programa.tipoId, tipo: masterMod.programa.tipo }
            } as any;
        }
    ...
    async getCategoriasModulo(moduloId: string, turnoId?: string) {
        ...
        if (!tipoId) {
            throw new NotFoundException('No se pudo determinar el tipo de programa para este módulo');
        }

        // 2. Obtener las configuraciones globales (plantilla) para este tipo de programa
        const configsGlobales = await this.prisma.mod_tipo_calificacion_config.findMany({
    ...
    async crearCategoria(
        userId: string,
        moduloId: string,
        data: { nombre: string; peso: number; esEvalFinal?: boolean; turnoId?: string },
    ) {
    ...
    async aplicarPlantilla(userId: string, moduloId: string, turnoId?: string) {
        ...
        if (!nuevas.length) {
            return {
                mensaje: 'Todas las categorías de la plantilla ya están aplicadas para este turno',
                aplicadas: 0,
        ...
        return {
            ...
            total: configs.length,
            categorias: nuevas.map(c => ({ id: c.id, nombre: c.nombre, peso: c.peso })),
        };
    }
    ...
    async actualizarCategoria(
        ...
        if (!instancia) throw new NotFoundException('Categoría no encontrada en este módulo');

        if (data.peso !== undefined) {
            await this.validatePeso(moduloId, data.peso, id, instancia.turnoId || undefined);
        }
