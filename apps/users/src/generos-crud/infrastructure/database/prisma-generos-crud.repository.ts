...
import { IGeneroRepository } from '../../domain/repositories/generos-crud.repository.interface';
...
export class PrismaGeneroRepository implements IGeneroRepository {
  ...
  async findAll(filter: any = {}, ability?: any): Promise<any[]> {
    ...
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'read', 'Genero');
      where = { AND: [where, caslWhere] };
    }

    return await (this.prisma as any).genero.findMany({
      where,
    });
  }
  ...
  async delete(id: string, userId?: string, ability?: any): Promise<void> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'delete', 'Genero');
      where = { AND: [where, caslWhere] };
    }
    const exists = await (this.prisma as any).genero.findFirst({ where });
    ...
    const hasStatus = true;
    if (hasStatus) {
      await (this.prisma as any).genero.update({
        where: { id },
        data: { estado: 'eliminado', deletedAt: new Date(), deletedBy: userId },
      });
    } else {
      await (this.prisma as any).genero.delete({ where: { id } });
    }
  }
}