...
export class PrismaUserRepository implements IUserRepository {
    ...
    async findById(id: string, ability?: any): Promise<User | null> {
        ...
        if (ability) {
            ...
            const user = await this.prisma.user.findFirst({ where: { AND: [caslWhere, where] }, include: USER_INCLUDE });
    ...
    async findAll(filter: any = {}): Promise<User[]> {
        ...
        const users = await this.prisma.user.findMany({ where, include: USER_INCLUDE, orderBy: { createdAt: 'desc' } });
    ...
    async create(data: any): Promise<User> {
        ...
        const created = await this.prisma.user.create({
            data: {
                ...userData,
                roles: roles ? { create: roles.map((roleId: string) => ({ roleId, modelType: 'App\\User' })) } : undefined,
                sedes: sedes ? { create: sedes.map((sedeId: string) => ({ sedeId })) } : undefined,
            },
    ...
    async update(id: string, data: any, ability?: any): Promise<User> {
        ...
        if (ability) {
            ...
            const target = await this.prisma.user.findFirst({ where: { AND: [caslWhere, { id }] } });
            if (!target) throw new Error('No tiene permisos para actualizar este usuario');
        }

        const updateData: any = { ...userData };
        if (roles !== undefined) {
            updateData.roles = { deleteMany: {}, create: roles.map((roleId: string) => ({ roleId, modelType: 'App\\User' })) };
        }
        if (sedes !== undefined) {
            updateData.sedes = { deleteMany: {}, create: sedes.map((sedeId: string) => ({ sedeId })) };
        }

        const updated = await this.prisma.user.update({ where: { id }, data: updateData, include: USER_INCLUDE });
    ...
    async delete(id: string, deletedBy?: string): Promise<void> {
        await this.prisma.user.update({
            where: { id },
            data: { estado: 'eliminado', deletedAt: new Date(), deletedBy: deletedBy || null },
        });
    }
    ...
    async getRawToken(id: string): Promise<string | null> {
        const user = await this.prisma.user.findUnique({ where: { id }, select: { resetPasswordToken: true } });
