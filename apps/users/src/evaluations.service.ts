...
export class EvaluationsService {
  ...
  async createEvaluation(
    ...
    this.logger.log(
      `Starting createEvaluation for data: ${JSON.stringify(data)} by admin: ${currentUserId} `,
    );
    ...
    const userId =
      sourceData.userId || sourceData.id_usuario || sourceData.user_id;
    const periodoId =
      sourceData.periodoId || sourceData.id_periodo || sourceData.periodo_id;
    const rawPuntajes =
      sourceData.puntajes || sourceData.scores || sourceData.id_criterio || []; // fallback case
    ...
    if (!userId || !isUuid(userId)) {
      this.logger.error(
        `Validation failed: userId invalid(${userId}) from body ${JSON.stringify(sourceData)}`,
      );
      throw new BadRequestException(
        `ID de usuario inválido o ausente: ${userId}.`,
      );
    }
    ...
    if (!periodoId || !isUuid(periodoId)) {
      ...
      throw new BadRequestException(
        `ID de período inválido o ausente: ${periodoId}.`,
      );
    }
    ...
    this.logger.log(
      `Normalizing rawPuntajes: ${JSON.stringify(normalizedRawPuntajes)}`,
    );
    ...
    if (puntajes.length === 0) {
      this.logger.warn(
        `No valid puntajes found in: ${JSON.stringify(normalizedRawPuntajes)}`,
      );
    ...
    const targetTenantId =
      responsableTenantId && isUuid(responsableTenantId)
        ? responsableTenantId
        : null;
    ...
    if (existing)
      throw new BadRequestException(
        'Ya existe una evaluación activa para este usuario en este período',
      );
    ...
    for (const p of puntajes) {
      ...
      if (!criterioIds.includes(p.criterioId))
        throw new BadRequestException(
          `Criterio inválido para este período: ${p.criterioId} `,
        );
      const crit = periodo.criterios.find((c: any) => c.id === p.criterioId);
      if (currentPuntaje < 0 || currentPuntaje > crit.puntajeMaximo)
        throw new BadRequestException(
          `Puntaje fuera de rango para el criterio ${crit.nombre}: ${currentPuntaje} (Max: ${crit.puntajeMaximo})`,
        );
    }

    const puntajeTotal = puntajes.reduce(
    ...
    try {
      ...
      this.logger.error(
        `DATABASE ERROR at createEvaluation: ${error.message}`,
        error.stack,
      );
      if (error.code === 'P2002') {
        throw new BadRequestException(
          'Ya existe una evaluación activa para este usuario en este período (P2002)',
        );
      }
      throw new BadRequestException(
        `Error al persistir la evaluación: ${error.message}`,
      );
    }
  }
