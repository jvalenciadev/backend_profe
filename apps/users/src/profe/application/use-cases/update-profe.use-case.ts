...
export class UpdateProfeUseCase {
  ...
  async execute(id: string, dto: UpdateProfeDto, userId?: string): Promise<Profe> {
    try {
      const existing = await this.repository.findById(id);
      if (!existing) {
        throw new NotFoundException(`Configuración institucional con ID ${id} no encontrada`);
      }
      ...
      throw new BadRequestException('Error al actualizar datos institucionales', { cause: error });
    }
  }
}
...
export class DeleteProfeUseCase {
  ...
  async execute(id: string, userId?: string): Promise<boolean> {
    try {
      ...
      if (error instanceof NotFoundException) throw error;
      throw new BadRequestException('Error al eliminar datos institucionales', { cause: error });
    }
  }
}
