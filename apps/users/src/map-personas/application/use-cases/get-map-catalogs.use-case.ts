  ...
  async getStats() {
    ...
    const [
      ...
      this.prisma.mapCargo.findMany({
        where: {
          id: {
            in: cargoStats.map((s) => s.carId).filter(Boolean) as string[],
          },
        },
      }),
      ...
      this.prisma.mapCategoria.findMany({
        where: {
          id: {
            in: categoriaStats.map((s) => s.catId).filter(Boolean) as string[],
          },
        },
      }),
      this.prisma.mapGenero.findMany({
        where: {
          id: {
            in: generoStats.map((s) => s.genId).filter(Boolean) as string[],
          },
        },
      }),
      this.prisma.mapArea.findMany({
        where: {
          id: {
            in: areaStats.map((s) => s.areaId).filter(Boolean) as string[],
          },
        },
      }),
      this.prisma.mapSubsistema.findMany({
        where: {
          id: {
            in: subsistemaStats.map((s) => s.subId).filter(Boolean) as string[],
          },
        },
      }),
      this.prisma.mapNivel.findMany({
        where: {
          id: {
            in: nivelStats.map((s) => s.nivId).filter(Boolean) as string[],
          },
        },
      }),
    ]);
    ...
    return {
      ...
      especialidades: especialidadStats.map((s) => ({
        name:
          especialidades.find((e) => e.id === s.espId)?.nombre ||
          'SIN ESPECIALIDAD',
        value: s._count._all,
      })),
      ...
      areas: areaStats.map((s) => ({
        name:
          areas.find((a) => a.id === s.areaId)?.nombre || 'ZONA NO DEFINIDA',
        value: s._count._all,
      })),
      subsistemas: subsistemaStats.map((s) => ({
        name:
          subsistemas.find((sub) => sub.id === s.subId)?.nombre || 'REGULAR',
        value: s._count._all,
      })),
      niveles: nivelStats.map((s) => ({
        name:
          niveles.find((n) => n.id === s.nivId)?.nombre || 'NIVEL NO DEFINIDO',
        value: s._count._all,
      })),
    };
  }
