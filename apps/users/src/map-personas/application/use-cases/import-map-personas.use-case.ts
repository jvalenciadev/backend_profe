...
export class ImportMapPersonasUseCase {
  ...
  private async processBatch(
    ...
    const cis = batch
      .map((row) => String(row['CI'] || row['ci'] || '').trim())
      .filter(Boolean);
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      const complemento = String(
        row['Complemento'] || row['complemento'] || row['COMPLEMENTO'] || '',
      )
        .trim()
        .toUpperCase();
      ...
      if (!ci) {
        job.errors.push({
          row: rowNum,
          ci: 'N/A',
          error: 'CI no encontrado en la fila',
        });
        continue;
      }
      ...
      try {
        ...
        // BOOLEANOS FLEXIBLES
        const enFuncion = this.parseBooleanFlexible(
          row['EnFuncion'] || row['en_funcion'] || row['Funcionando'],
        );
        const libretaMilitar = this.parseBooleanFlexible(
          row['LibretaMilitar'] || row['libreta_militar'] || row['Libreta'],
        );

        // BÚSQUEDA Y CREACIÓN DE CATÁLOGOS POR NOMBRE
        const carId = await this.getOrCreateCatalogId(
          this.prisma.mapCargo,
          row['Cargo'] || row['cargo'],
          catalogs.cargos,
        );
        const espId = await this.getOrCreateCatalogId(
          this.prisma.mapEspecialidad,
          row['Especialidad'] || row['especialidad'],
          catalogs.especialidades,
        );
        const catId = await this.getOrCreateCatalogId(
          this.prisma.mapCategoria,
          row['Categoria'] || row['categoria'],
          catalogs.categorias,
        );
        const nivId = await this.getOrCreateCatalogId(
          this.prisma.mapNivel,
          row['Nivel'] || row['nivel'],
          catalogs.niveles,
        );
        const subId = await this.getOrCreateCatalogId(
          this.prisma.mapSubsistema,
          row['Subsistema'] || row['subsistema'],
          catalogs.subsistemas,
        );
        const genId = await this.getOrCreateCatalogId(
          this.prisma.mapGenero,
          generoValue,
          catalogs.generos,
        );
        const areaId = await this.getOrCreateCatalogId(
          this.prisma.mapArea,
          row['Area'] || row['area'],
          catalogs.areas,
        );
        ...
        const personaData: any = {
          rda:
            row['RDA'] || row['rda'] ? BigInt(row['RDA'] || row['rda']) : null,
          nombre1: String(
            row['Nombre1'] || row['Nombre'] || row['nombre1'] || '',
          )
            .trim()
            .toUpperCase(),
          nombre2:
            row['Nombre2'] || row['nombre2']
              ? String(row['Nombre2'] || row['nombre2'])
                  .trim()
                  .toUpperCase()
              : null,
          apellido1: String(
          ...
          apellido2:
            ...
            row['APELLIDO_MATERNO']
              ? String(
                  ...
                  .toUpperCase()
              : null,
          fechaNacimiento: this.parseExcelDate(
          ...
          celular:
            row['Celular'] || row['celular']
              ? Number(row['Celular'] || row['celular'])
              : 0,
          correo:
            row['Correo'] || row['correo']
              ? String(row['Correo'] || row['correo']).trim()
              : '',
          carId,
          espId,
          catId,
        ...
        const existing = existingPersonas.find(
          (p) => p.ci === ci && (p.complemento || '') === (complemento || ''),
        );

        if (existing) {
          await this.prisma.mapPersona.update({
            where: { id: existing.id },
            data: personaData,
          });
          job.updated++;
        } else {
          await this.prisma.mapPersona.create({
            data: { ...personaData, ci, complemento },
          });
          job.success++;
        }
      } catch (err: any) {
  ...
  private parseBooleanFlexible(val: any): boolean {
    ...
    return ['SI', 'SÍ', 'S', 'TRUE', '1', 'VERDADERO', 'YES', 'Y'].includes(
      clean,
    );
  }
  ...
  private async loadCatalogs() {
    ...
    return {
      ...
      especialidades: new Map(
        especialidades.map((c) => [c.nombre.toUpperCase(), c.id]),
      ),
      categorias: new Map(
      ...
      subsistemas: new Map(
        subsistemas.map((c) => [c.nombre.toUpperCase(), c.id]),
      ),
  ...
  private async getOrCreateCatalogId(
    model: any,
    name: any,
    ...
    if (!record) {
      record = await model.create({
        data: { nombre: cleanName, estado: 'activo' },
      });
    }
