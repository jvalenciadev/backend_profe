...
export class GeneratePDFUseCase {
    ...
    async execute(id: string): Promise<Buffer> {
        ...
        const isResponsable = creator?.roles?.some((r: any) => r.role.name.includes('RESPONSABLE'));
        ...
        return new Promise((resolve, reject) => {
            ...
            if (bgPath) {
                try { doc.save(); doc.image(bgPath, 0, 0, { width: 612, height: 792 }); doc.restore(); }
                catch (e) { this.logger.error(`Error fondo_doc: ${e.message}`); }
            }
            ...
            if (user.imagen) {
                ...
                if (fs.existsSync(imgPath)) {
                    try { doc.image(imgPath, margin, 25, { width: 55, height: 65 }); }
                    catch (e) { this.logger.error(`Error imagen usuario: ${e.message}`); }
                }
            }
            ...
            doc.fontSize(7).fillColor(BLACK).text('Código de verificación:', qrX - 5, qrY + 67, { width: 80, align: 'center' });
            doc.fontSize(11).fillColor(RED).text(evaluation.codigoVerificacion, qrX - 5, qrY + 77, { width: 80, align: 'center' });
            ...
            // Title
            doc.font('Helvetica-Bold').fontSize(16).fillColor(BLACK).text('HOJA DE CONCEPTO', 0, 130, { align: 'center' });
            doc.fontSize(12).text(`CORRESPONDIENTE A LA GESTIÓN ${periodo?.gestion || '2025'}`, 0, 150, { align: 'center' });
            ...
            doc.text(
                ...
                margin, 200, { width: contentW, align: 'justify', lineGap: 2 },
            );
            doc.moveDown(0.8);
            ...
            for (const criterio of criterios) {
                ...
                const textHeight = doc.heightOfString(criterio.nombre, { width: colVal - colCrit - 12 });
                const rowH = Math.max(textHeight + 10, 24);

                doc.rect(colCrit, rowY, colVal - colCrit - 4, rowH).lineWidth(0.5).strokeColor(RED).stroke();
                doc.fontSize(9).fillColor(BLACK)
                    .text(`${rowNum}.`, colNum, rowY + 5, { width: 18, align: 'right' });
                doc.text(criterio.nombre, colCrit + 4, rowY + 5, { width: colVal - colCrit - 12, align: 'left', lineGap: 1 });
            ...
            // Total & date
            rowY += 14;
            doc.fontSize(10).fillColor(BLACK).text(`31 de diciembre de ${periodo?.gestion || '2025'}`, margin, rowY);
            doc.font('Helvetica-Bold').text('CALIFICACIÓN TOTAL:', colVal - 20, rowY, { continued: true }).text(`   ${evaluation.puntajeTotal}`);
            doc.font('Helvetica');
            ...
            doc.fontSize(7).fillColor('#FFFFFF').text(
                ...
                margin, pageH - 20, { width: contentW, align: 'center' },
            );
