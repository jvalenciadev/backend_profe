...
export class CreateEvaluacionUseCase {
  ...
  async execute(
    ...
    const alreadyExists = await this.repository.existsActiveForUserInPeriodo(
      userId,
      periodoId,
    );
    if (alreadyExists)
    ...
    const qrCode = await QRCode.toDataURL(
      `${frontendUrl}/verificar-evaluacion?code=${codigoVerificacion}`,
    );
