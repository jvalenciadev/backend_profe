...
export class BancoProfesionalService {
    ...
    async registrar(data: any) {
        ...
        const tenantIdValue = (data.tenantId && String(data.tenantId).trim() !== '') ? String(data.tenantId).trim() : null;

        // 2. Validaciones Críticas
        if (!ciRaw) throw new BadRequestException('El número de Cédula de Identidad (CI) es requerido.');
        if (!cargoIdValue) throw new BadRequestException('Debe seleccionar un cargo al que postula.');
        if (!data.correo) throw new BadRequestException('El correo electrónico es requerido.');
        if (!data.username) throw new BadRequestException('El nombre de usuario es requerido.');
        ...
        if (!verification || verification.code !== data.verificationCode || verification.expires < new Date()) {
        ...
        try {
            ...
            throw new BadRequestException('El formato del CI no es válido (solo números).');
        }
        ...
        if (conflict) {
            if (conflict.correo === data.correo) throw new ConflictException('Este correo ya está registrado.');
            if (conflict.username === data.username) throw new ConflictException('Este nombre de usuario ya está ocupado.');
            if (conflict.ci === ciBigInt) throw new ConflictException('Este número de CI ya está registrado.');
        }

        // 5. Verificación de Cargo
        const cargo = await this.prisma.cargo.findFirst({ where: { id: cargoIdValue } });
        if (!cargo) throw new NotFoundException('El cargo seleccionado no es válido o no existe.');

        // 6. Preparación de Seguridad (Rol)
        let role = await this.prisma.role.findFirst({ where: { name: 'POSTULACION_PROFE' } });
        if (!role) {
            role = await this.prisma.role.create({ data: { name: 'POSTULACION_PROFE', guardName: 'api' } });
        }

        const hashedPassword = await bcrypt.hash(data.password, 12);
        const birthDate = new Date(data.fechaNac);
        if (isNaN(birthDate.getTime())) {
            throw new BadRequestException('La fecha de nacimiento es inválida o tiene un formato incorrecto.');
        }
        ...
        try {
            return await this.prisma.$transaction(async (tx) => {
                ...
                const user = await tx.user.create({
                    data: {
                        ...
                        tieneProduccion: data.tieneProduccion === true || data.tieneProduccion === 'true',
            ...
            throw new BadRequestException('No se pudo completar el registro. Verifique que todos los datos sean correctos.');
        }
    }
    ...
    async findAll(filter: { cargoId?: string; estado?: string } = {}) {
        const where: any = { estado: { not: 'eliminado' }, cargoPostulacionId: { not: null } };
    ...
    async update(id: string, data: any, currentUserId: string) {
        console.log(`Updating ficha for user ${id} with data:`, JSON.stringify(data));
        ...
        try {
            ...
            if (data.correo && data.correo !== ficha.correo) {
                ...
                const verification = this.verificationCodes.get(data.correo);
                if (!verification || verification.code !== data.verificationCode || verification.expires < new Date()) {
                    throw new BadRequestException('El código de verificación para el nuevo correo es incorrecto o ha expirado.');
                }
            ...
            const allowedFields = [
                'esMaestro', 'licUniversitaria', 'licMescp', 'tieneProduccion',
                'hojaDeVidaPdf', 'estado',
                ...
                'celular', 'genero', 'direccion', 'estadoCivil', 'imagen'
            ];
            ...
            if (data.cargoId !== undefined) {
                ...
                updateData.cargoPostulacionId = (cargoId && cargoId !== 'null') ? cargoId : null;
            }

            console.log('Update Data Prepared:', JSON.stringify(updateData, (_, v) => typeof v === 'bigint' ? v.toString() : v));
            ...
            throw new BadRequestException('Error al actualizar la ficha: ' + error.message);
        }
    }
    ...
    async remove(id: string, currentUserId: string) {
        return this.prisma.user.update({
            where: { id },
            data: { estado: 'eliminado', deletedAt: new Date(), deletedBy: currentUserId }
        });
    }

    // ─── ACTUALIZAR ESTADO Y ASIGNAR ROL (APROBAR o DAR DE BAJA) ──────────────────
    async aprobar(id: string, data: { roleId: string; tenantId?: string; status?: string }, currentUserId: string) {
        const targetStatus = (data.status || 'activo').toLowerCase();
        console.log(`Updating professional ${id} to status: ${targetStatus} with roleId: ${data.roleId}`);
        ...
        return await this.prisma.$transaction(async (tx) => {
            ...
            const tenantId = (data.tenantId && data.tenantId.trim() !== '') ? data.tenantId : user.tenantId;
    ...
    // ─── POSTGRADOS CRUD ──────────────────────────────────────────────────────────
    async addPosgrado(userId: string, data: {
        tipoPosgradoId: string;
        titulo: string;
        fecha: string;
        imagen?: string;
    }, currentUserId: string) {
        console.log('DEBUG: addPosgrado called with:', { userId, tipoPosgradoId: data.tipoPosgradoId, titulo: data.titulo, currentUserId });

        if (!userId) throw new BadRequestException('El ID de usuario es requerido.');
        if (!data.tipoPosgradoId) throw new BadRequestException('El tipo de postgrado es requerido.');
        ...
        if (!isUuid) {
            ...
            if (found) {
                ...
                throw new BadRequestException(`El tipo de postgrado '${tipoId}' no es válido. Seleccione uno de la lista.`);
            }
        }
        ...
        try {
            ...
            if (error.code === 'P2003') {
                throw new BadRequestException('El tipo de postgrado o usuario no existe.');
            }
    ...
    async updatePosgrado(id: string, data: any, currentUserId: string) {
        ...
        if (data.tipoPosgradoId !== undefined) updateData.btp_id = data.tipoPosgradoId;
    ...
    async addProduccion(userId: string, data: {
        titulo: string;
        ...
        try {
            return await this.prisma.bp_produccion_intelectual.create({
                data: {
                    ...
                    bpi_anio_publicacion: data.anioPublicacion ? Number(data.anioPublicacion) : new Date().getFullYear(),
                    created_by: currentUserId,
    ...
    async updateProduccion(id: string, data: any, currentUserId: string) {
        ...
        if (data.anioPublicacion !== undefined) updateData.bpi_anio_publicacion = Number(data.anioPublicacion);
    ...
    async updateTipoPosgrado(id: string, data: { nombre?: string; estado?: string }) {
    ...
    // ─── CARGOS CRUD ──────────────────────────────────────────────────────────────
    async getCargos() {
        return this.prisma.cargo.findMany({ where: { estado: { not: 'eliminado' } } });
    }
    ...
    private serializeFicha(data: any) {
        ...
        const obj = JSON.parse(JSON.stringify(data, (_, v) =>
