...
export class PrismaBancoProfesionalRepository implements IBancoProfesionalRepository {
    ...
    async findByEmailOrUsername(email: string, username: string): Promise<BancoProfesional | null> {
    ...
    async updateProduccion(id: string, data: any): Promise<any> {
        return this.prisma.bp_produccion_intelectual.update({
            ...
            data: {
                bpi_titulo: data.titulo,
                bpi_anio_publicacion: data.anioPublicacion ? parseInt(data.anioPublicacion) : undefined,
                updated_at: new Date(),
                updated_by: data.updatedBy
            }
        });
    ...
    private mapToEntity(data: any): BancoProfesional {
        ...
        if (data.bp_produccion_intelectual) {
            entityData.produccionIntelectual = data.bp_produccion_intelectual.map((p: any) => ({
                ...p,
                id: p.bpi_id,
                titulo: p.bpi_titulo,
