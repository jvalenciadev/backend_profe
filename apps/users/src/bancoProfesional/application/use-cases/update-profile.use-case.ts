...
export class UpdateProfileUseCase {
    ...
    async execute(id: string, dto: UpdateProfileDto, currentUserId: string): Promise<BancoProfesional> {
        try {
            const existing = await this.repository.findById(id);
            if (!existing) throw new NotFoundException('Ficha profesional no encontrada');
            ...
            allowedFields.forEach((f) => {
                if ((dto as any)[f] !== undefined) {
                    if (f === 'esMaestro' || f === 'tieneProduccion') {
                        updateData[f] = (dto as any)[f] === true || (dto as any)[f] === 'true';
                    } else {
            ...
            throw new BadRequestException('Error al actualizar el expediente profesional', { cause: error });
        }
    }
}
