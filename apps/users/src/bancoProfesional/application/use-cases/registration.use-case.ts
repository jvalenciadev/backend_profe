...
export class RegistrationUseCase {
  ...
  async execute(data: any): Promise<any> {
    ...
    const ci = String(data.ci || '')
      .trim()
      .replace(/\D/g, '');
    const rda = String(data.rda || '')
      .trim()
      .replace(/\D/g, '');
    ...
    const activeConflict = conflicts.find(
      (c) => c.estado !== 'inactivo' && c.estado !== 'eliminado',
    );
    if (activeConflict) {
      if (activeConflict.correo === data.correo)
        throw new ConflictException(
          'Este correo ya está registrado en una cuenta activa.',
        );
      if (activeConflict.ci === ciBigInt)
        throw new ConflictException(
          'Este número de CI ya está registrado en una cuenta activa.',
        );
      if (activeConflict.username === data.username && data.username)
