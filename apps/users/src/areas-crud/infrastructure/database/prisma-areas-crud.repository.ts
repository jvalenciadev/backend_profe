...
import { IAreaRepository } from '../../domain/repositories/areas-crud.repository.interface';
...
export class PrismaAreaRepository implements IAreaRepository {
  ...
  async findAll(filter: any = {}, ability?: any): Promise<any[]> {
    ...
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'read', 'Area');
      where = { AND: [where, caslWhere] };
    }

    return await (this.prisma as any).areaTrabajo.findMany({
      where,
    });
  }
  ...
  async delete(id: string, userId?: string, ability?: any): Promise<void> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'delete', 'Area');
      where = { AND: [where, caslWhere] };
    }
    const exists = await (this.prisma as any).areaTrabajo.findFirst({ where });
    if (!exists)
    ...
    const hasStatus = true;
    if (hasStatus) {
      await (this.prisma as any).areaTrabajo.update({
        where: { id },
        data: { estado: 'eliminado', deletedAt: new Date(), deletedBy: userId },
      });
    } else {
      await (this.prisma as any).areaTrabajo.delete({ where: { id } });
    }
  }
}
