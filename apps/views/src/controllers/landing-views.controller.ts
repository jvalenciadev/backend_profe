import { Controller, Get, Post, Body, Query, BadRequestException, UseInterceptors, UploadedFile, Param } from '@nestjs/common';
...
export class LandingViewsController {
    ...
    async getLandingPageData(@Query('tenant') tenant?: string) {
        ...
        const [profe, eventos, programasRaw, comunicados, blogs, galerias, sedes, cargos] = await Promise.all([
            this.prisma.profe.findFirst({ where: { estado: 'activo' } }),
            this.prisma.evento.findMany({
                where: { estado: 'activo', ...(tenantId ? { tenantId } : {}) },
                take: 12, orderBy: { fecha: 'desc' },
                include: { tipo: true, cuestionarios: { where: { estado: 'activo' } } },
            }),
            this.prisma.programaDos.findMany({
                where: { estado: 'activo', ...(tenantId ? { departamentoId: tenantId } : {}) },
                orderBy: { createdAt: 'desc' },
                include: { tipo: true, modalidad: true, duracion: true, sede: true, version: true },
            }),
            ...
            this.prisma.galeria.findMany({
                where: { estado: 'activo', ...(tenantId ? { sede: { is: { departamentoId: tenantId } } } : {}) },
        ...
        return { profe, eventos, programas: uniqueProgramas, comunicados, blogs, galerias, sedes, cargos };
    }
    ...
    async checkPersonaByDate(
        ...
        if (persona && personaMatch) {
            ...
            if (!user) {
                const baseUsername = `${persona.ci}${persona.complemento || ''}`.toLowerCase();
                const existing = await this.prisma.user.findFirst({ where: { username: baseUsername } });
                const finalUsername = existing ? `${baseUsername}_${Date.now()}` : baseUsername;

                user = await this.prisma.user.create({
                    data: {
                        nombre: persona.nombre1 || 'Sin nombre',
                        apellidos: `${persona.apellido1 || ''} ${persona.apellido2 || ''}`.trim() || 'Sin apellido',
            ...
            return {
                ...
                alreadyEnrolled: programaId ? await this.checkEnrollment(user.id, programaId) : false,
            };
        }
        ...
        if (adminUser) {
            ...
            if (fechaMatch) {
                return {
                    ...
                    apellidoMaterno: adminUser.apellidos?.split(' ').slice(1).join(' ') || null,
                    ...
                    alreadyEnrolled: programaId ? await this.checkEnrollment(adminUser.id, programaId) : false,
                };
            }
        }
    ...
    private async checkEnrollment(userId: string, programaId: string): Promise<boolean> {
        ...
        if (prog.programaId && prog.versionId) {
            ...
            const ins = await this.prisma.programaInscripcion.findFirst({
                where: { personaId: userId, programaId: { in: hermanoIds }, estado: 'activo' }
            });
            return !!ins;
        } else {
            const ins = await this.prisma.programaInscripcion.findFirst({
    ...
    async registerInscripcion(@Body() body: any) {
        const { userId, programaId, turnoId, sedeId, baucher, datosAdicionales, datosPersona } = body;
        ...
        if (userId) {
            ...
            if (!datosPersona?.nombre || !datosPersona?.apellidos) {
                throw new BadRequestException('Nombre y apellidos son requeridos para personas sin padrón');
            }
            ...
            const alreadyExists = await this.prisma.user.findFirst({ where: { username: baseUsername } });
            const finalUsername = alreadyExists ? `${baseUsername}_${Date.now()}` : baseUsername;

            const correo = datosPersona.correo || `${finalUsername}@profe.edu.bo`;
            const existingEmail = await this.prisma.user.findFirst({ where: { correo } });
            const finalCorreo = existingEmail ? `${finalUsername}@profe.edu.bo` : correo;
        ...
        if (progHijo?.programaId && progHijo?.versionId) {
            ...
            if (yaInscrito) {
                throw new BadRequestException('Ya se encuentra inscrito en este programa.');
            }
        }
        ...
        if (!estadoId) {
            const fallback = await this.prisma.programa_inscripcion_estado.findFirst();
            if (!fallback) throw new BadRequestException('No hay estados de inscripción configurados');
        ...
        const alreadyHasRole = rolePart ? await this.prisma.userRole.findFirst({
            where: { userId: user.id, roleId: rolePart.id },
        }) : null;
        if (rolePart && !alreadyHasRole) {
            await this.prisma.userRole.create({
                data: { userId: user.id, roleId: rolePart.id, modelType: 'App\\Models\\User' },
            });
        }
        ...
        this.mailService.sendInscripcionConfirmation(
            user.correo,
            `${user.nombre} ${user.apellidos}`,
            progFull?.nombre || 'Programa PROFE',
            progFull?.sede?.nombre || 'Central'
        ).catch(err => console.error('Error enviando correo de inscripción:', err));
    ...
    async uploadPublicFile(@UploadedFile() file: any) {
        ...
        const finalPath = await this.uploadConfig.getDynamicPath(null, 'programa_baucher');
        ...
        return {
            ...
            data: { filename, path: fileUrl, size: file.size, mimetype: file.mimetype },
        };
    }
    ...
    async sendVerificationCode(@Body() body: { correo: string, nombre: string }) {
        ...
        if (!sent) {
            throw new BadRequestException('No se pudo enviar el correo de verificación. Verifique su dirección o intente más tarde.');
        }
    ...
    async verifyCode(@Body() body: { correo: string, code: string }) {
        ...
        if (!record) {
            throw new BadRequestException('No hay código de verificación pendiente para este correo');
        }
    ...
    async resetPasswordWithCode(@Body() body: { correo: string, code: string, password: unknown }) {
        const { correo, code, password } = body;
        if (!correo || !code || !password || typeof password !== 'string') {
            throw new BadRequestException('Faltan parámetros o la contraseña es inválida');
        }
        ...
        const user = await this.prisma.user.findUnique({ where: { correo: emailLower, estado: 'activo' } });
        if (!user) throw new BadRequestException('Usuario no encontrado o inactivo');
