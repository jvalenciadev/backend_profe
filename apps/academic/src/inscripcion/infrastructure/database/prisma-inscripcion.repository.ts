...
export class PrismaInscripcionRepository implements IInscripcionRepository {
    ...
    async findByPersonaAndPrograma(personaId: string, programaId: string): Promise<Inscripcion | null> {
    ...
    async checkTurnAvailability(programaId: string, turnoId: string): Promise<{ cupo: number, cupoPre: number } | null> {
    ...
    async create(data: any): Promise<Inscripcion> {
        ...
        const created = await this.prisma.programaInscripcion.create({
            data: {
                ...
                sedeId: data.sedeId,
                estadoInscripcionId: data.estadoInscripcionId || '05a0199e-76f8-4b72-97ad-e78a632128bd', // Example UUID if not provided
