  ...
  async execute(
    ...
    if (!oferta.isEnrollmentOpen()) {
      throw new BadRequestException(
        'El periodo de inscripción para esta oferta ha finalizado o no ha iniciado',
      );
    }
    ...
    if (existing) {
      throw new ConflictException(
        'La persona ya se encuentra inscrita en este programa',
      );
    }
    ...
    if (!reserved) {
      throw new BadRequestException(
        'No hay cupos disponibles o turno inválido para esta inscripción',
      );
    }
    ...
    try {
      ...
      throw new BadRequestException(
        'Error al registrar la inscripción. Intente de nuevo.',
        { cause: error },
      );
    }
  }
