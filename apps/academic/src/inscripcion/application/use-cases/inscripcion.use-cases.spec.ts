...
describe('Inscripcion Use Cases (Blindaje Completo)', () => {
  ...
  beforeEach(async () => {
    ...
    createUseCase = module.get<CreateInscripcionUseCase>(CreateInscripcionUseCase);
    updateUseCase = module.get<UpdateInscripcionUseCase>(UpdateInscripcionUseCase);
  });
  ...
  describe('CreateInscripcionUseCase', () => {
    it('debe lanzar NotFoundException si la oferta no existe', async () => {
      ...
      await expect(createUseCase.execute(dto)).rejects.toThrow(NotFoundException);
    });
    ...
    it('debe lanzar BadRequestException si el período de inscripción está cerrado', async () => {
      ...
      await expect(createUseCase.execute(dto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('debe lanzar ConflictException si la persona ya está inscrita', async () => {
      ...
      mockInscripcionRepo.findByPersonaAndPrograma.mockResolvedValue({ id: 'existing-ins' });
      await expect(createUseCase.execute(dto)).rejects.toThrow(ConflictException);
    });
  ...
  describe('UpdateInscripcionUseCase', () => {
    it('debe lanzar NotFoundException si la inscripción no existe', async () => {
      ...
      await expect(updateUseCase.execute('ins-x', {})).rejects.toThrow(NotFoundException);
    });
    ...
    it('debe actualizar datos sin cambiar programa', async () => {
      ...
      mockInscripcionRepo.update.mockResolvedValue({ ...existing, estado: 'confirmado' });

      const result = await updateUseCase.execute('ins-1', { estado: 'confirmado' }, 'admin-1');
      expect(result.id).toBe('ins-1');
      expect(mockInscripcionRepo.update).toHaveBeenCalledWith('ins-1', expect.objectContaining({ estado: 'confirmado', updatedBy: 'admin-1' }));
    });
    ...
    it('debe lanzar NotFoundException si el nuevo programa no existe al cambiar', async () => {
      ...
      await expect(updateUseCase.execute('ins-1', { programaId: 'prog-nuevo' })).rejects.toThrow(NotFoundException);
    });

    it('debe actualizar sedeId desde la nueva oferta al cambiar programa', async () => {
      const existing = { id: 'ins-1', programaId: 'prog-1' };
      mockInscripcionRepo.findById.mockResolvedValue(existing);
      mockOfertaRepo.findById.mockResolvedValue({ id: 'prog-2', sedeId: 'sede-nueva' });
      mockInscripcionRepo.update.mockResolvedValue({ id: 'ins-1', sedeId: 'sede-nueva' });
