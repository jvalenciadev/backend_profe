...
import {
  GetEventoPreguntasUseCase, GetEventoPreguntaByIdUseCase, CreateEventoPreguntaUseCase, UpdateEventoPreguntaUseCase, DeleteEventoPreguntaUseCase
...
export class EventoPreguntaController {
  ...
  create(@Body() data: any, @Req() req: any) {
    return this.createEventoPreguntaUseCase.execute(data, req.user?.id, req.user?.tenantId);
  }

  @Put(':id')
  @CheckPolicies((ability: any) => ability.can('update', 'EventoPregunta'))
  updatePut(@Param('id') id: string, @Body() data: any, @Req() req: any) {
    return this.updateEventoPreguntaUseCase.execute(id, data, req.user?.id, req.ability);
  }
  ...
  remove(@Param('id') id: string, @Req() req: any) {
    return this.deleteEventoPreguntaUseCase.execute(id, req.user?.id, req.ability);
  }
}