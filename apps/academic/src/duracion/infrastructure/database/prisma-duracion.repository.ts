...
import { IDuracionRepository } from '../../domain/repositories/duracion.repository.interface';

@Injectable()
export class PrismaDuracionRepository implements IDuracionRepository {
  constructor(
  ...
  async findAll(filter: any = {}, ability?: any): Promise<any[]> {
    ...
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'read', 'Duracion');
      where = { AND: [where, caslWhere] };
    }

    return await (this.prisma as any).programaDuracion.findMany({
      where,
  ...
  async update(id: string, data: any, userId?: string, ability?: any): Promise<any> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'update', 'Duracion');
      where = { AND: [where, caslWhere] };
    }
  ...
  async delete(id: string, userId?: string, ability?: any): Promise<void> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'delete', 'Duracion');
      where = { AND: [where, caslWhere] };
    }
    const exists = await (this.prisma as any).programaDuracion.findFirst({ where });
    ...
    const hasStatus = true;
    if (hasStatus) {
      await (this.prisma as any).programaDuracion.update({
        where: { id },
        data: { estado: 'eliminado', deletedAt: new Date(), deletedBy: userId },
      });
    } else {
      await (this.prisma as any).programaDuracion.delete({ where: { id } });
    }
  }
}