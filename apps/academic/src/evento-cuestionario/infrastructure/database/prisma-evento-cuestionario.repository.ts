...
export class PrismaEventoCuestionarioRepository implements IEventoCuestionarioRepository {
  ...
  async update(id: string, data: any, userId?: string, ability?: any): Promise<any> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'update', 'EventoCuestionario');
      where = { AND: [where, caslWhere] };
    }
    const exists = await (this.prisma as any).eventoCuestionario.findFirst({ where });
  ...
  async delete(id: string, userId?: string, ability?: any): Promise<void> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'delete', 'EventoCuestionario');
      where = { AND: [where, caslWhere] };
    }
    const exists = await (this.prisma as any).eventoCuestionario.findFirst({ where });
