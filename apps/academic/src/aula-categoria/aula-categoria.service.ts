...
export class AulaCategoriaService {
     ...
     */
    async createConfig(tipoProgramaId: string, data: {
        nombre: string;
        peso: number;
        esEvalFinal?: boolean;
        orden?: number;
        ...
        const tipo = await this.prisma.programaTipo.findUnique({ where: { id: tipoProgramaId } });
    ...
    async updateConfig(id: string, data: {
        ...
        const config = await this.prisma.mod_tipo_calificacion_config.findUnique({ where: { id } });
        if (!config) throw new NotFoundException('Configuración no encontrada');

        if (data.peso !== undefined) {
            const tipo = await this.prisma.programaTipo.findUnique({ where: { id: config.tipoProgramaId } });
            const maxNota = tipo?.notaMaxima || 100;

            // Validar que el peso total ajustado no supere el máximo
            const existentes = await this.prisma.mod_tipo_calificacion_config.findMany({
                where: { tipoProgramaId: config.tipoProgramaId, estado: 'activo', id: { not: id } },
            });
    ...
    async aplicarConfigAModulo(moduloId: string, tipoProgramaId: string) {
        let modulo: any = await this.prisma.programaModuloDos.findUnique({ where: { id: moduloId } });
        if (!modulo) {
            modulo = await this.prisma.programaModulo.findUnique({ where: { id: moduloId } });
        }
        ...
        if (!configs.length) throw new BadRequestException('No hay configuraciones para este tipo');
        ...
        if (nuevas.length === 0) {
            throw new BadRequestException('Todas las categorías ya están aplicadas a este módulo');
        }

        const isMaster = await this.prisma.programaModulo.findUnique({ where: { id: moduloId } });
     ...
     */
    async create(moduloId: string, data: { nombre: string; ponderacion: number; esEvalFinal?: boolean }) {
        ...
        if (modulo) {
            tipoId = (modulo.programaDos as any)?.tipoId;
            maxNota = (modulo.programaDos?.tipo as any)?.notaMaxima || 100;
        } else {
        ...
        if (!modulo) throw new NotFoundException('Módulo no encontrado');
        if (!tipoId) throw new NotFoundException('El programa no tiene un tipo configurado');
        ...
        const pesoActual = existentes.reduce((sum, c) => sum + (c.config?.peso || 0), 0);
     ...
     */
    async update(id: string, data: { nombre?: string; ponderacion?: number; esEvalFinal?: boolean }) {
        ...
        if (data.ponderacion !== undefined) {
            ...
            const pesoOtros = existentes.reduce((sum, c) => sum + (c.config?.peso || 0), 0);
        ...
        await this.prisma.mod_tipo_calificacion_config.update({
            ...
            data: {
                ...
                ...(data.ponderacion !== undefined && { peso: Number(data.ponderacion) }),
                ...(data.esEvalFinal !== undefined && { esEvalFinal: data.esEvalFinal }),
            },
        });

        return this.findAll((categoria.moduloId || categoria.moduloMaestroId) as string);
    }
    ...
    async remove(id: string) {
        const categoria = await this.prisma.mod_categoria_calificacion.findUnique({ where: { id } });
        if (!categoria) throw new NotFoundException('Categoría no encontrada');
