...
describe('AulaCategoriaService (Blindaje Completo - 459 líneas)', () => {
  ...
  describe('getConfigByTipos', () => {
    it('debe retornar tipos de programa con sus categorías', async () => {
      mockPrisma.programaTipo.findMany.mockResolvedValue([
        { id: 't1', nombre: 'Diplomado', mod_tipos_calificacion: [] },
      ]);
  ...
  describe('createConfig', () => {
    it('debe lanzar NotFoundException si el tipo no existe', async () => {
      ...
      await expect(
        service.createConfig('tipo-999', { nombre: 'Test', peso: 30 }),
      ).rejects.toThrow(NotFoundException);
    });

    it('debe lanzar BadRequestException si el peso excede el máximo', async () => {
      mockPrisma.programaTipo.findUnique.mockResolvedValue({
        id: 't1',
        notaMaxima: 100,
      });
      mockPrisma.mod_tipo_calificacion_config.findMany.mockResolvedValue([
      ...
      await expect(
        service.createConfig('tipo-1', { nombre: 'Extras', peso: 30 }),
    ...
    it('debe crear la config si el peso es válido', async () => {
      ...
      mockPrisma.mod_tipo_calificacion_config.create.mockResolvedValue({
        id: 'config-new',
      });
      const result = await service.createConfig('tipo-1', {
  ...
  describe('updateConfig', () => {
    it('debe lanzar NotFoundException si la config no existe', async () => {
      mockPrisma.mod_tipo_calificacion_config.findUnique.mockResolvedValue(
        null,
      );
      await expect(
        service.updateConfig('conf-999', { nombre: 'Nuevo' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('debe lanzar BadRequestException si el nuevo peso excede el máximo', async () => {
      ...
      await expect(service.updateConfig('c1', { peso: 30 })).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('deleteConfig', () => {
    it('debe lanzar NotFoundException si no existe', async () => {
      ...
      await expect(service.deleteConfig('conf-999')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('debe hacer soft delete (estado=inactivo)', async () => {
      ...
      expect(
        mockPrisma.mod_tipo_calificacion_config.update,
  ...
  describe('aplicarConfigAModulo', () => {
    it('debe lanzar NotFoundException si el módulo no existe', async () => {
      ...
      await expect(
        service.aplicarConfigAModulo('mod-999', 'tipo-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('debe lanzar BadRequestException si no hay configs para el tipo', async () => {
      ...
      mockPrisma.mod_tipo_calificacion_config.findMany.mockResolvedValue([]);
      await expect(
        service.aplicarConfigAModulo('mod-1', 'tipo-1'),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('debe crear categorías nuevas y retornar conteo', async () => {
      ...
      mockPrisma.mod_categoria_calificacion.createMany.mockResolvedValue({
        count: 2,
      });
  ...
  describe('getAllModulos', () => {
    it('debe retornar todos los módulos sin filtros', async () => {
      mockPrisma.programaModuloDos.findMany.mockResolvedValue([
        { id: 'm1' },
        { id: 'm2' },
      ]);
  ...
  describe('create', () => {
    it('debe lanzar NotFoundException si el módulo no existe', async () => {
      ...
      mockPrisma.programaModulo.findUnique.mockResolvedValue(null);
      await expect(
        service.create('mod-x', { nombre: 'X', ponderacion: 20 }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('debe crear categoría y config para un módulo de instancia', async () => {
      ...
      const result = await service.create('mod-1', {
        nombre: 'Examen',
        ponderacion: 50,
      });
      expect(result.id).toBe('cat-1');
      expect(
        mockPrisma.mod_tipo_calificacion_config.create,
      ).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ tipoProgramaId: 'tipo-1', peso: 50 }),
        }),
      );
    });
    ...
    it('debe crear categoría y config para un módulo maestro', async () => {
      ...
      const result = await service.create('mod-master', {
        nombre: 'Master Category',
        ponderacion: 30,
      });
      expect(result.id).toBe('cat-m');
      expect(mockPrisma.mod_categoria_calificacion.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ moduloMaestroId: 'mod-master' }),
        }),
      );
    });
    ...
    it('debe lanzar BadRequestException si el peso total excede el máximo', async () => {
      ...
      await expect(
        service.create('mod-1', { nombre: 'Examen', ponderacion: 30 }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('update', () => {
    it('debe lanzar NotFoundException si la categoría no existe', async () => {
      ...
      await expect(
        service.update('cat-x', { nombre: 'Nuevo' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('debe actualizar nombre y ponderación correctamente', async () => {
      ...
      mockPrisma.programaModuloDos.findUnique.mockResolvedValue({
        id: 'm1',
        programaDos: { tipo: { notaMaxima: 100 } },
      });
      ...
      expect(
        ...
        expect.objectContaining({
          where: { id: 'cfg-1' },
          data: expect.objectContaining({ nombre: 'Nuevo', peso: 40 }),
        }),
      );
    });
    ...
    it('debe lanzar BadRequestException si el nuevo peso excede el límite', async () => {
      ...
      await expect(
        service.update('cat-1', { ponderacion: 20 }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('remove', () => {
    ...
    it('debe hacer soft delete de la categoría', async () => {
      ...
      mockPrisma.mod_categoria_calificacion.update.mockResolvedValue({
        id: 'cat-1',
        estado: 'inactivo',
      });
