...
import { IVersionRepository } from '../../domain/repositories/version.repository.interface';
...
export class PrismaVersionRepository implements IVersionRepository {
  ...
  async findAll(filter: any = {}, ability?: any): Promise<any[]> {
    ...
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'read', 'Version');
      where = { AND: [where, caslWhere] };
    }

    return await (this.prisma as any).programaVersion.findMany({
      where,
  ...
  async update(id: string, data: any, userId?: string, ability?: any): Promise<any> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'update', 'Version');
      where = { AND: [where, caslWhere] };
    }
  ...
  async delete(id: string, userId?: string, ability?: any): Promise<void> {
    let where: any = { id };
    if (ability) {
      const caslWhere = this.caslPrisma.getWhere(ability, 'delete', 'Version');
      where = { AND: [where, caslWhere] };
    }
    const exists = await (this.prisma as any).programaVersion.findFirst({ where });
    ...
    const hasStatus = true;
    if (hasStatus) {
      await (this.prisma as any).programaVersion.update({
        where: { id },
        data: { estado: 'eliminado', deletedAt: new Date(), deletedBy: userId },
      });
    } else {
      await (this.prisma as any).programaVersion.delete({ where: { id } });
    }
  }
}