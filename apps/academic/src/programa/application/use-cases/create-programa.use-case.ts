...
export class CreateProgramaUseCase {
  ...
  async execute(data: any): Promise<Programa> {
    const { fechaInicioInscripcion, fechaFinInscripcion, fechaInicioClases } = data;
    ...
    if (fechaInicioInscripcion && fechaFinInscripcion) {
      ...
      if (endInsc <= startInsc) {
        throw new BadRequestException('La fecha de fin de inscripción debe ser posterior a la fecha de inicio.');
      }

      if (fechaInicioClases) {
        const startClasses = new Date(fechaInicioClases);
        if (startClasses < endInsc) {
          throw new BadRequestException('La fecha de inicio de clases no puede ser anterior al fin de inscripciones.');
        }
      }
    }
