...
export class GenericCrudService<T> {
  ...
  constructor(
    ...
    this.caslSubject =
      caslSubject || modelName.charAt(0).toUpperCase() + modelName.slice(1);
  }
    ...
    try {
      ...
      const cleanCriteria = (obj: any, fieldsToRemove: string[]): any => {
        ...
        if (Array.isArray(obj))
          return obj.map((item) => cleanCriteria(item, fieldsToRemove));
