...
export class CaslPrismaService {
    ...
    getWhere(ability: AppAbility, action: string, subject: string) {
        try {
            ...
            let criteria = rules[subject] || rules.all || {};
    ...
    private cleanCriteria(criteria: any, subject: string): any {
        ...
        const globalSubjects = [
            ...
            'MapPersona', 'AreaTrabajo', 'Genero', 'Provincia', 'UnidadEducativa', 'ActaConclusion', 'Profe', 'ProgramaInscripcionEstado',
            'ProgramaBaucher', 'ProgramaRestriccion', 'CalificacionParticipante', 'ProgramaCalificacion', 'ProgramaTipoCalificacion',
            'EventoRestriccion', 'EventoCuestionario', 'EventoPregunta', 'EventoOpcion', 'EventoRespuesta', 'Galeria'
        ];
    ...
    private recursiveClean(obj: any, fieldsToRemove: string[]): any {
        ...
        if (Array.isArray(obj)) return obj.map(item => this.recursiveClean(item, fieldsToRemove));
