      ...
      data: {
        ...
        minWidth:
          data.minWidth !== undefined
            ? data.minWidth
              ? parseInt(data.minWidth)
              : null
            : undefined,
        maxWidth:
          data.maxWidth !== undefined
            ? data.maxWidth
              ? parseInt(data.maxWidth)
              : null
            : undefined,
        minHeight:
          data.minHeight !== undefined
            ? data.minHeight
              ? parseInt(data.minHeight)
              : null
            : undefined,
        maxHeight:
          data.maxHeight !== undefined
            ? data.maxHeight
              ? parseInt(data.maxHeight)
              : null
            : undefined,
      },
    });
  ...
  async validateImage(tableName: string, file: any): Promise<boolean> {
    ...
    if (!config || config.estado !== 'activo') {
      ...
      if (file.size > maxSizeBytes) {
        throw new BadRequestException(
          `El archivo excede el tamaño máximo permitido (${envMaxSize}MB)`,
        );
      }
      return true;
    }
    ...
    try {
      ...
      if (fileInfo) {
        ...
        const isOfficeDoc =
          ['docx', 'xlsx', 'pptx'].includes(fileExt) && detectedExt === 'zip';

        // Excepción de jpg/jpeg
        const isJpegVar =
          (fileExt === 'jpg' && detectedExt === 'jpg') ||
          (fileExt === 'jpeg' && detectedExt === 'jpg');

        if (!isOfficeDoc && !isJpegVar && detectedExt !== fileExt) {
          throw new BadRequestException(
            `Format mismatch. Pretende ser ${fileExt} pero internamente es ${detectedExt}`,
          );
        }
      }
    } catch (err: any) {
    ...
    if (imageExtensions.includes(fileExt)) {
      try {
        ...
        if (
          ...
          throw new BadRequestException(
            `El ancho de la imagen es muy pequeño. Mínimo: ${config.minWidth}px (Actual: ${metadata.width}px)`,
          );
        }
        if (
          config.maxWidth &&
          ...
          throw new BadRequestException(
            `El ancho de la imagen es muy grande. Máximo: ${config.maxWidth}px (Actual: ${metadata.width}px)`,
          );
        }
        if (
          config.minHeight &&
          ...
          throw new BadRequestException(
            `El alto de la imagen es muy bajo. Mínimo: ${config.minHeight}px (Actual: ${metadata.height}px)`,
          );
        }
        if (
          config.maxHeight &&
          ...
          throw new BadRequestException(
            `El alto de la imagen es muy alto. Máximo: ${config.maxHeight}px (Actual: ${metadata.height}px)`,
          );
        }
        ...
        if (config.aspectRatio && metadata.width && metadata.height) {
          ...
          if (Math.abs(currentRatio - targetRatio) > tolerance) {
            throw new BadRequestException(
              `La relación de aspecto de la imagen no coincide con la rquerida (${config.aspectRatio})`,
            );
          }
        }
      } catch (err) {
        ...
        throw new BadRequestException(
          'No se pudo procesar la imagen para validación. Verifique el formato.',
        );
      }
    }
